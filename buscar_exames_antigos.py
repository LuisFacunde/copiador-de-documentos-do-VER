...
from src.parser import extrair_info_arquivo_antigo
...
from analisar_exames import descobrir_prontuarios
...
def _coletar_exames_prontuario_antigo_lista(
    arquivos: list[Path],
) -> list[dict]:
    exames = []
    ...
    for arquivo in arquivos:
        ...
        data_exame, pront, tipo = extrair_info_arquivo_antigo(arquivo)
        if tipo is None:
            continue
...
def buscar_exames_antigos(
    dir_origem: str = DIRS_EXAMES_ANTIGOS,
    ...
    print("\n" + "=" * 70)
    print("  🔄 BUSCA DE EXAMES ANTIGOS — PRONTUÁRIOS JÁ COPIADOS")
    print("=" * 70)
    ...
    print("\n  🔍 Iniciando varredura no diretório de origem (isso pode demorar)...")
    ...
    # Filtrar apenas os que estão na tabela historico_copias
    prontuarios_alvo = {
        p: arqs
        for p, arqs in prontuarios_arquivos.items()
        if p in prontuarios_copiados
    }

    pendentes = sorted(list(prontuarios_alvo.keys()))
    total_pendentes = len(pendentes)
    
    print(f"  📋 Prontuários alvo encontrados no diretório: {total_pendentes}")

    if total_pendentes == 0:
        print("  ✅ Nenhum dos prontuários procurados foi encontrado na origem.")
        conn.close()
        return

    numero_lote = obter_proximo_numero_lote(conn)
    data_envio = datetime.now().strftime("%d-%m-%Y")
    nome_lote = f"Busca Antigos {numero_lote} - {data_envio}"
    ...
    caminho_log = pasta_lote / f"log_busca_antigos_{numero_lote}.txt"
    logger = configurar_logger(caminho_log, numero_lote)

    logger.info("=" * 70)
    logger.info(f"  🔄 BUSCA DE EXAMES ANTIGOS — LOTE {numero_lote}")
    ...
    for analisados, prontuario in enumerate(pendentes, 1):
        prefixo = f"[{analisados}/{total_pendentes}]"

        arquivos_prontuario = prontuarios_alvo.get(prontuario, [])
        exames = _coletar_exames_prontuario_antigo_lista(arquivos_prontuario)

        if not exames:
            logger.info(
                f"{prefixo} Prontuário {prontuario} 📭 "
                f"Sem exames RET/OCT antigos identificados"
            )
        ...
        if not pendentes_copia:
            if pulados_prontuario > 0:
                logger.info(f"{prefixo} Prontuário {prontuario} ⏭️  {pulados_prontuario} arquivo(s) já existente(s) nas bases")
...
def main():
    parser = argparse.ArgumentParser(
        description="Busca de Exames Antigos — Procura exames (RET/OCTPAPILA) "
        "com nomenclatura antiga para prontuários já copiados",
    )
    parser.add_argument(
        "--origem",
        type=str,
        default=DIRS_EXAMES_ANTIGOS,
        help=f"Diretório de origem dos exames antigos (padrão: {DIRS_EXAMES_ANTIGOS})",
    )
    parser.add_argument(
        "--destino",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Reprocessa todos os prontuários forçadamente",
    )
    parser.add_argument(
        "--silencioso",
        action="store_true",
